...
# Download YOLOv5 repository and model if not available
...
# Check if YOLOv5 repo and weights are available, download if missing
if not (ROOT / 'models').exists():
    print("Downloading YOLOv5 models...")
    os.system(f"git clone {YOLOV5_REPO} {ROOT}")
if not (ROOT / 'yolov5s.pt').exists():
    print("Downloading YOLOv5 weights...")
    os.system(f"curl -L {MODEL_URL} -o {ROOT / 'yolov5s.pt'}")
...
def run(
    ...
    LOGGER.info(f'Speed: {t[0]:.1f}ms pre-process, {t[1]:.1f}ms inference, {t[2]:.1f}ms NMS per image at shape {im.shape}')
